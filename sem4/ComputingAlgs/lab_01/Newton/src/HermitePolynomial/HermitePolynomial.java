...
//    private double yDer[] = {6, 8, 10, 12, 14, 16, 18};
    private double x[] = {3, 3, 4, 4, 5, 5, 6, 6, 7, 7, 8, 8, 9, 9};
    //      private double y[] = {-4, 1, 0, -1, 0, 1};
    private double y[] = {9, 9, 16, 16, 25, 25, 36, 36, 49, 49, 64, 64, 81, 81};
    private double yDer[] = {6, 6, 8, 8, 10, 10, 12, 12, 14, 14, 16, 16, 18, 18};
...
//    private double y[] = {0.00, 1.00, 4.00, 9.00, 16.00, 25.00, 36.00};
    ...
    public HermitePolynomial(int powPolynomial, double argValue) {
    ...
    public double getFunctionValue() {
        ...
        for (int i = 0; i < indexEnd - indexStart; i++) {
            ...
            for (int j = 0; j < indexEnd - indexStart - i; j++) {
                ...
                System.out.println("x[j + indexstart] = " + x[j + indexStart] + " x[j + istart + i] =  " + x[j + indexStart + i]);
//                System.out.println("dif / dif = " + (temp_y[j + 1] - temp_y[j]) / (x[j + 1 + i] - x[j]));

                if (x[j + indexStart] - x[indexStart + j + i ] == 0) {
                    System.out.println("HERE tempy[j]=  " + yDer[j + indexStart]);
                    temp_y[j] = yDer[j + indexStart];
                }
