...
int main(int argc, char *argv[])
{
    ...
    int check_args = parse_args(argc, argv, input_file_name, output_file_name, &is_filter);
    ...
    if (is_filter)
    {
        ...
        int res = key(pb_src, pe_src, &edges);
        ...
        mem_alloc(&pb_dst, (edges.right - edges.left - 1));
    
        filter(edges.left, edges.right, pb_dst, &pe_dst);
